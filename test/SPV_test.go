...
func TestSPV(t *testing.T) {
	...
	for idx, test := range spvTests {
		...
		for num, findidx := range test.findTX {
			...
			ok, route, hashroute := testBlock.MTree.Find(primeTXs[findidx].ID)
			if ok {
			...
			spvRes := merkletree.SPV(primeTXs[findidx].ID, testBlock.MTree.RootNode.HashData, route, hashroute)
